		...
		onVariantChangeHandler(e){
			...
			if ( this.priceCompact ) {
				if ( !variant ) {
					...
					this.priceCompact.priceOriginal.innerHTML = this.productVariants._formatMoney(variant.price, theme_custom_data.settings.shop_money_format);
					if ( variant.compare_at_price > variant.price ) {
						this.priceCompact.priceCompare.innerHTML = this.productVariants._formatMoney(variant.compare_at_price, theme_custom_data.settings.shop_money_format);
						//this.priceCompact.priceCompare.innerHTML = `<span>${this.productVariants._formatMoney(variant.compare_at_price, theme_custom_data.settings.shop_money_format)}</span><span class="product-item__badge">-${Math.round(Math.abs((variant.price * 100 / variant.compare_at_price) - 100))}%</span>`;
					} else {
						this.priceCompact.priceCompare.innerHTML = '';
					}
					...
					if ( variant.unit_price_measurement ) {
						this.priceCompact.priceUnit.innerHTML = `
							${this.productVariants._formatMoney(variant.unit_price, theme_custom_data.settings.shop_money_format)} / 
			...
			if ( this.priceExtended ) {
				if ( !variant ) {
					...
					this.priceExtended.priceOriginal.innerHTML = this.productVariants._formatMoney(variant.price, theme_custom_data.settings.shop_money_format);
					if ( variant.compare_at_price > variant.price ) {
						this.priceExtended.priceCompare.innerHTML = `<span>${this.productVariants._formatMoney(variant.compare_at_price, theme_custom_data.settings.shop_money_format)}</span><span>${theme_custom_data.settings.locales.product_compare_price.replace('$SAVE_PRICE', this.productVariants._formatMoney(variant.compare_at_price - variant.price, theme_custom_data.settings.shop_money_format))}</span>`;
					} else {
						this.priceExtended.priceCompare.innerHTML = '';
					}
